...
class NN_Base(object):
    ...
    def _conv(self, x, filters, kernel_size, strides, name, padding='SAME',
              trainable=True):
        x = tf.layers.conv2d(inputs=x, filters=filters,
                             kernel_size=kernel_size, strides=strides,
                             padding=padding, trainable=trainable, name = name)
        return x

    def _deconv(self, x, filters, kernel_size, strides, name, padding = 'valid',
                trainable = True):
        x = tf.layers.conv2d_transpose(inputs = x, filters = filters,
                                       kernel_size = kernel_size, strides = strides,
                                       padding = padding, name = name, trainable = trainable)
        return x

    def _max_pool(self, x, pool_size, name):
        x = tf.layers.max_pooling2d(x, pool_size, strides=(2, 2), name = name)
        return x
    ...
    def _fully_connected(self, x, out_dim, name, use_bias=True):
        x = tf.layers.dense(x, out_dim, name = name, use_bias=use_bias)
        return x
